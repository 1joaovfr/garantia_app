...
class VisualizacaoTab(ttk.Frame):
    def __init__(self, parent_notebook, main_app):
        ...
        ## --- CORREÇÃO AQUI --- ##
        # Renomeado de _criar_widgets para _create_widgets para manter o padrão
    ...
    def aplicar_filtros(self):
        ...
        for item in resultados:
            ...
            ressarc = f"R$ {float(item['ressarcimento']):.2f}" if item.get('ressarcimento') and str(item['ressarcimento']).replace('.','',1).isdigit() else "-"
            self.tree.insert("", END, values=(
                item['id'], item['numero_nota'], data, 
                item['cnpj'], item['cliente'], item['codigo_analise'] or '-', 
                item['codigo_produto'], valor, item['status'], 
                item['procedente_improcedente'] or '-', ressarc
            ))
    ...
    def _excluir_itens(self):
        ...
        if Messagebox.yesno(f"Tem certeza que deseja excluir as {len(ids)} peças selecionadas?", "Confirmação de Exclusão") == "Yes":
            sucesso, msg = self.service.excluir_itens(ids)
            if sucesso:
                Messagebox.show_info(msg, "Sucesso"); self.main_app.notificar_atualizacao()
            else:
    ...
    def _editar_item(self):
        ...
        if not selecionados or len(selecionados) > 1:
            Messagebox.show_warning("Selecione apenas uma peça para editar.", "Aviso"); return
