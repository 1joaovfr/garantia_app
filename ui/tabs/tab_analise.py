...
class AnaliseTab(ttk.Frame):
    ...
    def carregar_itens_pendentes(self):
        ...
        for item in itens:
            ...
            self.tree_analise.insert("", END, values=(item['id'], item['codigo_analise'], item['numero_nota'], item['cliente'], item['codigo_produto'], data_formatada, ressarcimento_str))
    ...
    def _atualizar_status_procedencia(self, event=None):
        ...
        info = self.codigos_avaria_map.get(cod_avaria, {}); classificacao = info.get('classificacao', '-')
    ...
    def _salvar_analise(self):
        ...
        for nome, valor in campos.items():
            if not valor.strip(): Messagebox.showerror(f"O campo '{nome}' é obrigatório.", "Erro de Validação"); return
        cod_avaria = campos['Cód. Avaria']
        dados = {'codigo_analise': self.analise_cod_entry.get(), 'numero_serie': campos['Nº de Série'], 'codigo_avaria': cod_avaria, 'descricao_avaria': self.analise_desc_avaria_entry.get(), 'procedente_improcedente': self.codigos_avaria_map.get(cod_avaria, {}).get('classificacao'), 'produzido_revenda': campos['Origem'], 'fornecedor': campos['Fornecedor']}
        sucesso, msg = self.service.salvar_analise(self.id_item_selecionado, dados)
        if sucesso:
            Messagebox.showinfo(msg, "Sucesso"); self._limpar_form(); self.main_app.notificar_atualizacao()
        else:
    ...
    def _set_form_state(self, state):
        for widget in self.form_frame.winfo_children():
            try:
                if isinstance(widget, (ttk.Entry, ttk.Combobox, ttk.Button)): widget.config(state=state)
                if isinstance(widget, ttk.Entry) and widget in [self.analise_cod_entry, self.analise_desc_avaria_entry]: widget.config(state="readonly" if state == "normal" else "disabled")
