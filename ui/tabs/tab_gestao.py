...
# A classe AutocompleteEntry permanece a mesma
class AutocompleteEntry(ttk.Entry):
    ...
    def _confirm_selection(self, event):
        ...
        if self._listbox.curselection():
            ...
            self._listbox.selection_set(tk.ACTIVE)
            self._on_listbox_select(event)
        return "break"
    def _on_listbox_select(self, event):
        ...
        value = self._listbox.get(selections[0])
        self.var.set(value)
        self.icursor(tk.END)
        self._hide_popup()
    def _on_click_outside(self, event):
        if self._popup and self._popup.winfo_exists():
            if event.widget != self and event.widget.master != self._popup:
                self._hide_popup()
    def _hide_popup(self):
        if self._popup and self._popup.winfo_exists():
            self._popup.winfo_toplevel().unbind("<Button-1>")
            self._popup.destroy()
            self._popup = None
...
class GestaoTab(ttk.Frame):
    ...
    def _criar_widgets(self):
        ...
        ## REMOVIDO: A criação do botão de exportar foi movida para dentro de _criar_view_tabela
        
        self.filtros_frame = ttk.LabelFrame(self, text="Filtros", padding=10)
        ...
        ttk.Label(self.filtros_frame, text="Grupo:").pack(side=LEFT, padx=(5,2))
    ...
    def _exportar_para_excel(self):
        ...
        if not dados_atuais:
            Messagebox.show_warning("Não há dados na tabela para exportar.", "Tabela Vazia")
            return
    ...
    def _mostrar_view(self, view_name):
        ...
        ## REMOVIDO: A lógica de mostrar/esconder o botão de exportar foi removida daqui,
        ## pois o botão agora vive dentro da própria 'frame_tabela'.
        if view_name == 'tabela':
    ...
    def atualizar_visualizacao(self):
        if self.current_view == 'tabela':
            filtros = {}
            self._atualizar_tabela(filtros)
        else:
